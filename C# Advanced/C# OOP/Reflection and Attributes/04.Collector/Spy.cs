...
namespace Stealer
{
    public class Spy
    {
        public string CollectGettersAndSetters(string className)
        {
            ...
            foreach (var getterMethod in methods.Where(x => x.Name.StartsWith("get")))
            {
                sb.AppendLine($"{getterMethod.Name} will return {getterMethod.ReturnType}");
            }

            foreach (var setterMethod in methods.Where(x => x.Name.StartsWith("set")))
            {
                sb.AppendLine($"{setterMethod.Name} will return {setterMethod.GetParameters().First().ParameterType}");
            }
        ...
        public string RevealPrivateMethods(string className)
        {
            ...
            MethodInfo[] privateMethods = 
                metadata.GetMethods(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
            StringBuilder sb = new StringBuilder();
            
            sb.AppendLine($"All Private Methods of Class: {metadata.FullName}");
            sb.AppendLine($"Base Class: {metadata.BaseType.Name}");
